 ...
 * against the person's title/body/tags embedding. Differs from `find_journey`, which takes a *named*
 * figure and returns the parks on their cross-park trail. Results link to the person's related parks.
 */
export default defineTool({
  description:
    "Find historical figures by a descriptive query (e.g. 'people connected to photography', 'figures from the conservation movement'). Semantic, not exact-name — use find_journey when the user names a specific person. Returns people with their related parks.",
