...
import { journeyTrail, journeyThemes } from '../../lib/queries';
import { ParkCard } from '../../components/ParkCard';
import { ThemeChips, type ThemeChipItem } from '../../components/journeys/ThemeChips';
import { TrailMiniGraph } from '../../components/graph/TrailMiniGraph';
import { PageHeader } from '../../components/ui/page-header';
import { SectionHeading } from '../../components/ui/section-heading';

/**
 * Journeys (NPS-expansion P0 #2, formerly `/trails`) — RSC. A "journey" is the set of parks connected by a
 * historical figure (`Person`-[:ASSOCIATED_WITH]->`Park`) or a shared `Topic`: a multi-hop graph traversal
 * no single park page reveals. Pick a theme → see the cross-park journey → fly the 3D tour or open /graph.
 * (Real hikeable trails now live at `/trails`; ADR-070.)
 */
export const dynamic = 'force-dynamic';

type SP = Record<string, string | undefined>;

export default async function JourneysPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  const [themes, trail] = await Promise.all([
    ...
    journeyThemes(500),
    selected ? journeyTrail({ person, topic }) : Promise.resolve([]),
  ]);

  const graphHref = person ? `/graph?person=${encodeURIComponent(person)}` : topic ? `/graph?topic=${encodeURIComponent(topic)}` : '/graph';
  const tourHref = person ? `/journeys/tour?person=${encodeURIComponent(person)}` : topic ? `/journeys/tour?topic=${encodeURIComponent(topic)}` : '/journeys/tour';

  const peopleChips: ThemeChipItem[] = themes.people.map((p) => ({
    key: p.title,
    label: p.title,
    parks: p.parks,
    href: `/journeys?person=${encodeURIComponent(p.title)}`,
  ...
  const topicChips: ThemeChipItem[] = themes.topics.map((t) => ({
    ...
    label: t.name,
    parks: t.parks,
    href: `/journeys?topic=${encodeURIComponent(t.name)}`,
    active: topic === t.name,
  }));
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Cross-park journeys connected by the people who shaped them or a theme they share — each one a single graph traversal. Looking for hikeable trails? They're at /trails."
        contour
      />
      ...
      <Container maxW="6xl" px={{ base: 4, md: 8 }} py={{ base: 8, md: 10 }}>
        ...
        {selected ? (
          <Box>
            <SectionHeading
              title={person ? `Parks tied to ${person}` : `${topic} journey`}
            ...
            {trail.length === 0 ? (
              ...
              <>
                {/* Fly the journey in 3D before the static grid (#11B). */}
                ...
                {/* See the journey as a connected graph before the card grid (ADR-039, friction #5). */}
                <Box mb={6}>
                  <TrailMiniGraph
                    themeLabel={selected}
          ...
          <Text color="fg.muted">Pick a person or topic above to trace its journey across the parks.</Text>
        )}
      </Container>
    </Box>
  );
