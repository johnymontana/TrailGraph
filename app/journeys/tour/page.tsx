...
import { journeyTrail, lessonPlansForPark } from '../../../lib/queries';
import { StoryTour, type TourStop } from '../../../components/journeys/StoryTour';

/**
 * Scrollytelling 3D journey tour (#11B): fly park-to-park across a journey (person/topic) on a sticky terrain
 * map as the reader scrolls narrative panels. Mirrors `app/journeys/page.tsx`'s person|topic params + reuses
 * `journeyTrail` for the ordered parks and `lessonPlansForPark` for a per-park "learn more" tie-in. The map
...
export default async function JourneyTourPage({ searchParams }: { searchParams: Promise<SP> }) {
  const sp = await searchParams;
  const person = sp.person?.trim() || undefined;
  const topic = sp.topic?.trim() || undefined;
  const selected = person || topic;

  const trail = selected ? await journeyTrail({ person, topic }) : [];
  ...
  if (!selected || stops.length === 0) {
    return (
      <Container maxW="2xl" py={{ base: 16, md: 24 }} textAlign="center">
        <Heading size="lg" mb={3}>Take a 3D tour of a journey</Heading>
        <Text color="fg.muted" mb={6}>
          {selected ? 'This theme has no mappable parks to fly through yet.' : 'Pick a person or topic to fly its journey across the parks.'}
        </Text>
        <CLink asChild color="brand.fg" fontWeight="medium">
          <NextLink href="/journeys">Browse journeys →</NextLink>
        </CLink>
      </Container>
    );
  }
